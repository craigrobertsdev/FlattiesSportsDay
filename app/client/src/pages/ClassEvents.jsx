...
function EventCard({ event, onSaved }) {
  const [cards, setCards]   = useState(
    () => HOUSE_NAMES.map(h => {
      ...
      return { house_name: h, athletic_points: existing?.athletic_points ?? 0, spirit_points: existing?.spirit_points ?? 0 };
    })
  );
  ...
  const handleSave = async () => {
    if (cards.some(c => c.athletic_points === 0)) { setError(true); return; }
    setSaving(true);
    try {
  ...
  return (
    <div className={`rounded-2xl border shadow-sm overflow-hidden ${saved ? 'border-green-200 bg-green-50' : 'border-slate-200 bg-white'}`}>
      {/* Title bar */}
      <div className={`flex items-center justify-between px-3 py-2 border-b ${saved ? 'border-green-200 bg-green-100' : 'border-slate-100 bg-slate-50'}`}>
        <h3 className="font-bold text-sm text-slate-800">{event.name}</h3>
        {saved
          ? <span className="text-xs font-bold text-green-700 bg-green-200 rounded-full px-2 py-0.5">✓ Saved</span>
        ...
        {HOUSE_NAMES.map(houseName => {
          ...
          return (
            <div key={houseName} className="flex items-center gap-2 px-3 py-1.5">
              ...
              {saved ? (
                <>
                  <span className="flex-1 text-right text-xs text-slate-600">Athletic: <strong>{card.athletic_points}</strong></span>
                  <span className="text-right text-xs text-slate-600">Spirit: <strong>{card.spirit_points}</strong></span>
                </>
              ) : (
                <>
                  <select
